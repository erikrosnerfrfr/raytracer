...
public class Main {
    ...
    IMaterial darkmirror = new MirrorMaterial(new Color(0.2,0.2,0.3), white, 100);
     ...
     scene.add (new PointLight(vec3(0,0,-6), new tools.Color(1,1,10)));
...
//////////////////////////////////////////////////////
   ...
   // scene.add(new Sphere(vec3(0,0,-5), 2, new TransmissiveMaterial(1.5)));
    ...
    Group cube = new Group(new Transform(multiply(move(0,0,-6), rotate(Vec3.yAxis, 20), scale(2,2,2))));
    ...
    scene.animators.add(new LinearRotator(pidgeon,Vec3.yAxis, 0, 360));

    for (int i = 0; i < numFrames; i ++) {

                scene.update(i * frequency);
    ...
    image.writePNG("a11-animation"+i);
